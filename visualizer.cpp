...
void Visualizer::handle_key_pressed(const sf::Keyboard::Key code) noexcept
{
	...
	case sf::Keyboard::I:
		m_view.change_real(c_factor);
		break;
	case sf::Keyboard::K:
		m_view.change_real(-c_factor);
		break;
	case sf::Keyboard::O:
		m_view.change_imaginary(c_factor);
		break;
	case sf::Keyboard::L:
		m_view.change_imaginary(-c_factor);
		break;
...
{    
    if (m_need_redraw) 
	{
		m_calculator.calculate(m_view.center(), m_view.width(), m_view.height(), m_view.c_real(), m_view.c_imaginary());
    }
}
...
void Visualizer::render()
{
    ...
    ss << 
		...
		"C: Re[" << m_view.c_real() << "]; Im[" << m_view.c_imaginary() << "]";
